public class MoneyExchange {
    ...
    public void CalMoney(double converter) {

        int FrontNum = 0;
        double DecimalNum = 0;

        //Split Integer & Decimal number
        if (converter % 1 != 0) {
            double SplitBack = converter % 1;
            DecimalNum = SplitBack;
        }

        double SplitFront = converter - DecimalNum;
        FrontNum = Convert.ToInt32(SplitFront);
